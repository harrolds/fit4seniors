...
export const RemindersView: React.FC<RemindersViewProps> = ({ variant = 'page' }) => {
  ...
  const handleSave = () => {
    ...
    if (selectedDays.length === 0 || validSlots.length === 0) {
      openBottomSheet('reminders-toast', { messageKey: 'validation.required' });
      return;
    }
  ...
  const containerClass = variant === 'sheet' ? 'reminders-sheet' : 'reminders-page';
              ...
              return (
                <div key={rule.id} className="reminders-time">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      removeRule(rule.id);
                      openBottomSheet('reminders-toast', { messageKey: 'reminders.toast.removed' });
                    }}
                  >
